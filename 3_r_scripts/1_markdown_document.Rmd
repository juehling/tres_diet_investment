...
To assess how nestling diet (via relative abundance) compares to adult diet, we used a mixed effect model with fixed effects age, experimental treatment, age*experimental treatment, and site, and random effect nest box. Not 100% sure this is the best approach yet, but this is what we're working with right now. 
...
#plot(mod_q2_ra) # Assumptions met

tab_model(mod_q2_ra)
```

It looks like there are not site differences here, but there are clear differences in how parents are allocating aquatic insects based on the CORT treatment they received, which is super interesting!! Note that these results may change with the addition of 60 Turkey Hill nestlings, all of which were part of the CORT dosing experiment.
...
Unclear exactly what is going on here...the light treatment has some type of effect, which is surprising. Let's just keep this model on the back burner for now, as again, it will likely change with the new data, and we are not using presence/absence models anyway.
...
```{r, echo = FALSE}
# Relative abundance
plot(percent_aquatic_ra_nestlings_mean~percent_aquatic_ra, data = aquatic_prov_comp, 
